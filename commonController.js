...
export default class CreateCommonController {
	constructor (Model) {
		...
		this.reservedKeys = ['fields', 'populate', 'sort', 'limit', '__fields', '__populate', '__sort', '__limit', 'on']
	...
	modifiers(query) {
		return {
			fields: JSON.parse(query.fields || query.__fields || '[]'),
			populate: JSON.parse(query.populate || query.__populate || '[]'),
			sort: JSON.parse(query.sort || query.__sort || '{}'),
			limit: Number(query.limit || query.__limit || '0'),
		}
	}
	...
	buildFilter(query) {
		...
		for (const key in query) {
			if (key.substring(0, 2) === '__' && !this.reservedKeys.includes(key)) {
				if (/__regex$/.test(key)) {
					filter[key.replace(/__regex$/, '').replace('__', '')] = {'$regex': new RegExp(query[key].replace(/\//g, ''))}
				} else if (/__json$/.test(key)) {
					filter[key.replace(/__json$/, '').replace('__', '')] = JSON.parse(query[key])
				} else {
					filter[key.replace('__', '')] = query[key]
				}
				...
				} else if (/^_j_/.test(key)) {
					filter[key.replace(/^_j_/, '')] = JSON.parse(query[key])
				} else {
	...
	verbPut(request, response) {
		...
		this.Model.findByIdAndUpdate(id, param, {new: true})
		.then(model => this.postUpdateMiddleware(param, model))
		.then(model => response.json(model))
		.catch(error => response.json({error}))
	}
